...
const getTaskBreakdown = (task: string) => {
  return {
    ...
    agents: [
      {
        agent: agents[0], // Brand Agent
        responsibility: '校验品牌调性，确保内容符合品牌DNA',
        output: '品牌指南 & 风格约束',
        isCore: true,
      },
      {
        agent: agents[1], // Strategy
        responsibility: '分析目标受众，制定营销策略',
        output: '营销方案 & 预算分配',
      },
      {
        agent: agents[2], // Copywriter
        responsibility: '撰写各平台文案内容',
        output: '文案素材库',
      },
      {
        agent: agents[3], // Designer
        responsibility: '设计配套视觉素材',
        output: '图片/视频素材',
      },
      {
        agent: agents[4], // Campaign
        responsibility: '一键多渠道发布内容',
        output: '发布排期 & 执行',
      },
      {
        agent: agents[5], // Analytics
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <a
              href="/cmo"
              className="flex items-center gap-2 bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 text-white px-4 py-2 rounded-lg text-sm font-semibold transition"
            >
              <span>CMO 视角</span>
